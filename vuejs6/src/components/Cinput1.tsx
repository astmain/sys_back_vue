...
function func(props: Props, { emit }: any) {
  const { type, placeholder, value, prefix, suffix, css } = toRefs(props)
  ...
  const onInput = (event: any) => {
    ...
    console.log("111---css", JSON.parse(JSON.stringify(css?.value)))
    emit("update:value", input_value.value)
  }

  return () => (
    <div class={clazz.parent} style={{ borderColor: isFocused.value ? "#409eff" : "", ...css?.value }}>
      {prefix?.value && (
      ...
      <input class={clazz.input} type={type?.value || "text"} placeholder={placeholder?.value || "请输入内容"} value={input_value.value} onInput={onInput} onFocus={onFocus} onBlur={onBlur} />
    </div>
  )
}
