...
/** 通用 paginate 函数 */
export async function paginate<T, A extends FindManyArgs>(model: { findMany(args: A): Promise<T[]>; count(args?: { where?: any }): Promise<number> }, baseArgs: A, pagination: PagePagination | CursorPagination<T>): Promise<PaginatedResult<T>> {
  if (pagination.mode === 'page') {
    ...
    return { data, meta: { mode: 'page', page, pageSize, total, totalPages, hasPrev: page > 1, hasNext: page < totalPages } }
  }
  ...
  return { data, meta: { mode: 'cursor', hasNext, hasPrev: Boolean(pagination.cursor), nextCursor } }
}
